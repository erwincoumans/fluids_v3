...
*/
	...
	//	virtual float GetValue ( float x, float y, float z, Vector3DF& dir );
		...
		unsigned int* getNeighborTable ( int n, int& cnt );
	...
	protected:
		...
		// Neighbor Table
		unsigned int				m_NC[1000000];			// neighbor table (600k)
		unsigned int				m_Neighbor[1000000][MAX_NEIGHBOR];	
		float						m_NDist[1000000][MAX_NEIGHBOR];
