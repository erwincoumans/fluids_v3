...
int GeomX::AddAttribute ( uchar parent_buf, std::string name, ushort stride, bool bExtend )
{
	GeomBuf* buf = &mBuf[parent_buf];
	GeomAttr attr;
...
void GeomX::ResetBuffer ( uchar b, int n )
{
	...
	printf ( "Buffer allocated: max %d, size %d\n", mBuf[b].max, mBuf[b].max*mBuf[b].stride );	
	char* new_data = (char*) malloc ( mBuf[b].max * mBuf[b].stride );	
	if ( new_data == 0x0 ) {
		error.PrintF ( "geom", "Unable to allocate memory.\n" );
		error.Exit ();
	}		
...
char* GeomX::AddElem ( uchar b, href& ndx )
{
	if ( mBuf[b].num >= mBuf[b].max ) {
		...
		mBuf[b].max *= 2;		
		printf ( "Buffer re-allocated: max %d, size %d\n", mBuf[b].max, mBuf[b].max*mBuf[b].stride );
		char* new_data = (char*) malloc ( mBuf[b].max * mBuf[b].stride );
