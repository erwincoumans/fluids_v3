...
{			
	...
	register float d2 = simData.psimscale * simData.psimscale;	
	register float r2 = simData.r2/d2;	
	register float srad = simData.psmoothradius;
	register float vterm = simData.lapkern * simData.pvisc;
	register float spiky = simData.spikykern;
	register float ss = simData.psimscale;
	register float dsq, pterm, dterm;
	register float3 dist, force;		
	register int j, cell, cfirst, clast;;	

	force = make_float3(0,0,0);		

	for (int c=0; c < simData.gridAdjCnt; c++) {
		force += contributeForce ( i, ipos, iveleval, ipress, idens, gc + simData.gridAdj[c], buf );

		/*cell = gc + simData.gridAdj[c];
		cfirst = buf.mgridoff[ cell ];
		clast = cfirst + buf.mgridcnt[ cell ];		
		for ( int cndx = cfirst; cndx < clast; cndx++ ) {										
			dist = ( ipos - buf.mpos_sort[cndx] );				// dist in cm
			dsq = (dist.x*dist.x + dist.y*dist.y + dist.z*dist.z);
			if ( dsq < r2 && dsq > 0) {
				j = buf.mgrid[ cndx ];						
				dsq = sqrt(dsq*d2);
				c = ( srad - dsq ); 
				pterm = ss * -0.5f * c * spiky * ( ipress + buf.mpress[j] ) / dsq;
				dterm = c * idens * (buf.mdensity[j] );
				//force += ( pterm * dist + vterm * ( buf.mveleval[j] - iveleval )) * dterm;
			}
		}
